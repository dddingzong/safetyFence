...
public class PaypassDeleteDuplicateAlgorithm {

    public Map<List<PaypassGeofence>, List<String>> algorithmStart(Map<String, List<Long>> averageTimeGeofenceMap, List<PaypassGeofence> paypassGeofenceList) {
        log.info("PaypassDeleteDuplicateAlgorithm 알고리즘 시작");

        // 데이터 정렬(fenceInTime 기준)
        List<PaypassGeofence> sortedGeofenceLocations = sortByUserFenceInTime(paypassGeofenceList);

        // routeId : List<> stationNumber 형식의 Map 작성
        Map<String, List<Long>> stationMap = makeStationMap(sortedGeofenceLocations, averageTimeGeofenceMap);
        log.info("stationMap = " + stationMap);

        // routeId : List<> geofenceLocation 형식의 Map 작성
        // route별 geofenceLocation Map 생성
        Map<String, List<PaypassGeofence>> geofenceLocationMap = makeGeofenceLocationMap(sortedGeofenceLocations, stationMap);
        log.info("geofenceLocationMap = " + geofenceLocationMap);

        // 인자가 하나인 경우 삭제 +
        // stationMap과 똑같은 모양의 routeId = List<GeofenceLocation> 형식의 Map 생성
        Map<String, List<PaypassGeofence>> continuousGeofenceLocationMap = makeContinuousGeofenceLocationMap(geofenceLocationMap, stationMap);
        ...
        Map<List<PaypassGeofence>, List<String>> deleteMap = makeResultMap(continuousGeofenceLocationMap, stationMap, timeMap);
        log.info("resultMap = " + deleteMap);

        return deleteMap;
    }

    private Map<List<PaypassGeofence>, List<String>> makeResultMap(Map<String, List<PaypassGeofence>> continuousGeofenceLocationMap, Map<String, List<Long>> stationMap, Map<String, List<Map<String, LocalDateTime>>> timeMap) {
        Map<List<PaypassGeofence>, List<String>> resultMap = new HashMap<>();
        log.info("continuousGeofenceLocationMap = " + continuousGeofenceLocationMap);
    ...
    private boolean checkDuplicateGeofenceLocation(Map<List<PaypassGeofence>, List<String>> resultMap, List<PaypassGeofence> geofenceLocationList) {
        Set<List<PaypassGeofence>> keySet = resultMap.keySet();
        for (List<PaypassGeofence> geofenceLocations : keySet) {
    ...
    private Map<String, List<Long>> makeStationMap(List<PaypassGeofence> geofenceLocations, Map<String, List<Long>> averageTimeMap) {
        ...
        for (var routeIdAndSequences : stationMap.entrySet()) {
            ...
            for (Long sequence : sequences) {
                ...
                for (PaypassGeofence geofenceLocation : geofenceLocations) {
                    if (geofenceLocation.getBusInfo().contains(oneBusInfo)) {
    ...
    private Map<String, List<PaypassGeofence>> makeGeofenceLocationMap(List<PaypassGeofence> geofenceLocations,  Map<String, List<Long>> stationMap) {
        ...
        geofenceLocationMapPlusValue(geofenceLocationMap, geofenceLocations, stationMap);

        return geofenceLocationMap;
    }

    private Map<String, List<Map<String, LocalDateTime>>> makeTimeMap(Map<String, List<PaypassGeofence>> continuousGeofenceLocationMap) {
    ...
    private Map<String, List<PaypassGeofence>> makeContinuousGeofenceLocationMap(Map<String, List<PaypassGeofence>> geofenceLocationMap, Map<String, List<Long>> stationMap) {
        ...
        for (var routeIdAndGeofenceLocationList : geofenceLocationMap.entrySet()) {
            ...
            for (String routeIdInRouteIdList : routeIdList){
                ...
                while (true) {
                    ...
                    log.info("routeIdInStationMap = " + routeIdInStationMap);

                    if (stationMap.containsKey(routeIdInStationMap)) {
                        log.info("-> 생성될 routeId 입니다.");
                    }

                    if (!stationMap.containsKey(routeIdInStationMap)) {
                        log.info("-> 삭제 예정 routeId 입니다");
                        break;
                    }
...
//                            System.out.println(" ");
                            ...
                            List<PaypassGeofence> putList = makePutList(idListForPut ,geofenceLocationList);
    ...
    private List<Long> makeStationNumberList(List<PaypassGeofence> geofenceLocationList) {
        return geofenceLocationList.stream()
                .map(PaypassGeofence::getStationNumber)
                .collect(Collectors.toList());
    }

    private List<Long> makeIdList(List<PaypassGeofence> geofenceLocationList) {
        return geofenceLocationList.stream()
                .map(PaypassGeofence::getId)
                .collect(Collectors.toList());
    }

    private List<PaypassGeofence> makePutList(List<Long> idListForPut, List<PaypassGeofence> geofenceLocationList) {
        List<PaypassGeofence> putList = geofenceLocationList.stream()
                .filter(geofenceLocation -> idListForPut.contains(geofenceLocation.getId())) // ID가 포함된 경우만 필터링
    ...
    private Map<String, List<Map<String, LocalDateTime>>> transformGeofenceLocationToTime(Map<String, List<PaypassGeofence>> continuousGeofenceLocationMap) {
    ...
    private void deleteContainStation(Map<String, List<Long>> stationMap, Map<String, List<Map<String, LocalDateTime>>> timeMap, Map<String, List<PaypassGeofence>> continuousGeofenceLocationMap) {
        ...
        for (var routeIdAndStationNumber : stationMap.entrySet()) {
            ...
            for (var routeIdAndStationNumberInner : stationMap.entrySet()) {
                ...
                // 포함은 하지만 동일하지 않는 경우
                System.out.println("stationNumberList = " + stationNumberList);
                System.out.println("stationNumberListInner = " + stationNumberListInner);
//                if (Collections.indexOfSubList(stationNumberList, stationNumberListInner) != -1 && !stationNumberList.equals(stationNumberListInner)) {
                if (stationNumberList.containsAll(stationNumberListInner) && !stationNumberList.equals(stationNumberListInner)) {
...
//        System.out.println("fenceOutTimeInner = " + fenceOutTimeInner);
    ...
    private List<String> makeBusInfoList(List<PaypassGeofence> sortedGeofenceLocations) {
        // busInfo만 존재하는 List 생성
        List<String> busInfoList = new ArrayList<>();

        for (PaypassGeofence geofenceLocation : sortedGeofenceLocations) {
            String busInfo = geofenceLocation.getBusInfo();
    ...
    private void geofenceLocationMapPlusKey(Map<String, List<PaypassGeofence>> geofenceLocationMap, Set<String> routeIdSet) {
        for (String routeId : routeIdSet) {
            geofenceLocationMap.put(routeId, new ArrayList<>());
        }
    }

    private void geofenceLocationMapPlusValue(Map<String, List<PaypassGeofence>> geofenceLocationMap, List<PaypassGeofence> geofenceLocations, Map<String, List<Long>> stationMap) {
        Map<String, List<Long>> mergedStationMap = mergeByRouteId(stationMap);

        for (var routeIdAndGeofenceLocationList : geofenceLocationMap.entrySet()) {
            String routeId = routeIdAndGeofenceLocationList.getKey();
            List<PaypassGeofence> geofenceLocationList = routeIdAndGeofenceLocationList.getValue();
            List<Long> stationNumberList = mergedStationMap.get(routeId);

            if (stationNumberList == null) {continue;}

            String pattern = ".*" + routeId + ".*";

            for (PaypassGeofence geofenceLocation : geofenceLocations) {

                if (geofenceLocation.getBusInfo().matches(pattern) && stationNumberList.contains(geofenceLocation.getStationNumber())) {
    ...
    private Map<String, List<Long>> mergeByRouteId(Map<String, List<Long>> stationMap) {
        Map<String, List<Long>> mergedMap = new TreeMap<>();

        for (Map.Entry<String, List<Long>> entry : stationMap.entrySet()) {
            String keyPrefix = entry.getKey().split("_")[0];
            List<Long> value = entry.getValue();

            mergedMap.computeIfAbsent(keyPrefix, k -> new ArrayList<>())
                    .addAll(value);
        }
